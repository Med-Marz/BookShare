...
const PROTO_FILE = 'book.proto';
const PROTO_PACKAGE = 'book';
const PROTO_SERVICE = 'BookService';
const GRPC_PORT = Number.parseInt(process.env.BOOK_GRPC_PORT || '50052', 10);
const HEALTH_PORT = Number.parseInt(process.env.BOOK_HEALTH_PORT || '50062', 10);
...
async function start() {
  const { snapshotPath } = await db.init(logger);
  ...
  // ---- Graceful shutdown — flush the snapshot before exiting ----
...
start().catch((err) => {
  logger.fatal({ err }, 'failed to start book-service');
  process.exit(1);
});
